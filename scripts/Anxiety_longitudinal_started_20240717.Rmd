...
#get list of scans with good Mimosa, n = 2263
mimosa_qc_gr_75 <- mimosa_qc_df %>%
  filter(score >= 75) %>%
  mutate(EXAM_DATA = as.character(EXAM_DATE)) %>%
  mutate(EMPI = as.character(EMPI)) %>%
  dplyr::select(EMPI, EXAM_DATE, score) %>%
  distinct(EMPI, EXAM_DATE, .keep_all = TRUE) %>%
  ungroup 
```

```{r merge_df_with_mimosa}

#n=770, 1078 scans total
df_people_with_good_mimosa <- data_2023_pull %>% 
  distinct(EMPI, EXAM_DATE, .keep_all = TRUE) %>%
  group_by(EMPI) %>%
  arrange(EMPI, EXAM_DATE) %>%
  filter(row_number() <= 2) %>%
  left_join(mimosa_qc_gr_75, by = c("EMPI", "EXAM_DATE")) %>%
  filter(!is.na(score))

# Check the number of rows per EMPI and keep people with only 2, n scans=616, with n=308 subjects
df_people_with_2_rows <- df_people_with_good_mimosa %>%
  group_by(EMPI) %>%
  filter(n() == 2) %>%
  ungroup()

# View the result, n = 308, with 616 total scans
print(df_people_with_2_rows)
print(length(df_people_with_2_rows$EMPI))
print(length(unique(df_people_with_2_rows$EMPI)))
```
...
df_demo_and_fascicles <- df_people_with_good_mimosa %>% 
  ...
  group_by(EMPI) %>% # again verify that this only includes people with two days
  filter(n() == 2) %>%
  mutate(exam_timepoint = ifelse(row_number() == 1, "t1", "t2"))

#check num unique EMPIs, n = 281
print(length(unique(df_demo_and_fascicles$EMPI)))

#check total n = 562
print(length(df_demo_and_fascicles$EMPI))

```

## Check to see the consistency within anxiety dose between t1 and t2
```{r consistency_of_anxiety}
# Create a summary to check for consistency, 127 people have two different anxiety doses
anxiety_dose_check <- df_demo_and_fascicles %>%
  group_by(EMPI) %>%
  summarize(
    unique_doses = n_distinct(anxiety_dose),
    doses = paste(unique(anxiety_dose), collapse = ", ")
  ) %>%
  ungroup()

# Filter to find EMPIs with more than one unique anxiety dose
inconsistent_doses <- anxiety_dose_check %>%
  filter(unique_doses > 1)

# Display the result
inconsistent_doses

#there are some, will take the higher value because if at some point you had disease, it should count as a lifetime history
df_demo_and_fascicles <- df_demo_and_fascicles %>%
  group_by(EMPI) %>%
  mutate(lifetime_max_anxiety_dose = max(anxiety_dose))

#now check inconsistent doses
# Create a summary to check for consistency, 127 people have two different anxiety doses
anxiety_dose_check <- df_demo_and_fascicles %>%
  group_by(EMPI) %>%
  summarize(
    unique_doses = n_distinct(lifetime_max_anxiety_dose),
    doses = paste(unique(anxiety_dose), collapse = ", ")
  ) %>%
  ungroup()
...
# Display the result - nothing there! It worked! Also checked this with my eyeballs
inconsistent_doses
...
##########################################################
#merge whole data set with fascicle proportions, n = 809
df_demo_and_fascicles <- df_demo_and_fascicles %>%
  left_join(lesion_volumes, by = c("EMPI", "EXAM_DATE")) 

#check num unique EMPIs, n = 809
print(length(unique(df_demo_and_fascicles$EMPI)))
...
df_w_yeo_volume_info <- df_demo_and_fascicles %>%
  ...
  dplyr::select(EMPI, EXAM_DATE, ends_with("_vol")) %>%
  ungroup()
...
# Longitudinal Age*Disease

## Anxiety Diagnosis
```{r diagnosisXage}

#n healthy = 155 total, 122 unique, +minA = 131
#define group
...
#overall - NS
diagnosis_by_age <- lmer(proportion_volume_lost_per_total_network_size_sum ~ Diagnosis*PAT_AGE_AT_EXAM + (1|EMPI), data = df_demo_and_fascicles_no_unclass_anxiety_dose)
summary(diagnosis_by_age)

### anxiety net
diagnosis_by_age_anx_net <- lmer(proportion_volume_lost_anxiety_net_by_network_size_sum_top_25 ~ Diagnosis*PAT_AGE_AT_EXAM + (1|EMPI), data = df_demo_and_fascicles_no_unclass_anxiety_dose)
summary(diagnosis_by_age_anx_net)

###nonanxiety net
diagnosis_by_age_nonanx_net <- lmer(proportion_volume_lost_nonanxiety_net_by_network_size_sum_bottom_75 ~ Diagnosis*PAT_AGE_AT_EXAM + (1|EMPI), data = df_demo_and_fascicles_no_unclass_anxiety_dose)
summary(diagnosis_by_age_nonanx_net)

### UF_R
diagnosis_by_age_UF_R <- lmer(UF_R_vol ~ Diagnosis*PAT_AGE_AT_EXAM + (1|EMPI), data = df_demo_and_fascicles_no_unclass_anxiety_dose)
summary(diagnosis_by_age_UF_R)

### UF_L
diagnosis_by_age_UF_L <- lmer(UF_L_vol ~ Diagnosis*PAT_AGE_AT_EXAM + (1|EMPI), data = df_demo_and_fascicles_no_unclass_anxiety_dose)
summary(diagnosis_by_age_UF_L)

### mean UF
diagnosis_by_age_mean_UF <- lmer(mean_UF_vol ~ Diagnosis*PAT_AGE_AT_EXAM + (1|EMPI), data = df_demo_and_fascicles_no_unclass_anxiety_dose)
summary(diagnosis_by_age_mean_UF)
```

## Anxiety Dose
```{r diagnosisXage}
#n healthy = 135, +minA = 131, severeA = 60

#define group
df_demo_and_fascicles_no_unclass_anxiety_dose <- df_demo_and_fascicles %>%
  mutate(EMPI = as.factor(EMPI)) %>%
  filter(oanxietydx_OR_meds_AND_Anxietydx.Meds.And.Dep.True.Healthy.Collapsed != "unclassified") %>%
  filter(lifetime_max_anxiety_dose != -1) %>%
  mutate(anxiety_dose = as.numeric(lifetime_max_anxiety_dose)) %>%
...
### diagnosis by age interaction?
#overall - NS
diagnosis_by_age <- lmer(proportion_volume_lost_per_total_network_size_sum ~ anxiety_dose*PAT_AGE_AT_EXAM + (1|EMPI), data = df_demo_and_fascicles_no_unclass_anxiety_dose)
summary(diagnosis_by_age)

diagnosis_by_age_factor <- lmer(proportion_volume_lost_per_total_network_size_sum ~ anxiety_dose_factor*PAT_AGE_AT_EXAM + (1|EMPI), data = df_demo_and_fascicles_no_unclass_anxiety_dose)
summary(diagnosis_by_age_factor)

### anxiety net
diagnosis_by_age_anx_net <- lmer(proportion_volume_lost_anxiety_net_by_network_size_sum_top_25 ~ anxiety_dose*PAT_AGE_AT_EXAM + (1|EMPI), data = df_demo_and_fascicles_no_unclass_anxiety_dose)
summary(diagnosis_by_age_anx_net)

###nonanxiety net
diagnosis_by_age_nonanx_net <- lmer(proportion_volume_lost_nonanxiety_net_by_network_size_sum_bottom_75 ~ anxiety_dose*PAT_AGE_AT_EXAM + (1|EMPI), data = df_demo_and_fascicles_no_unclass_anxiety_dose)
summary(diagnosis_by_age_nonanx_net)

### UF_R
diagnosis_by_age_UF_R <- lmer(UF_R_vol ~ anxiety_dose*PAT_AGE_AT_EXAM + (1|EMPI), data = df_demo_and_fascicles_no_unclass_anxiety_dose)
summary(diagnosis_by_age_UF_R)

### UF_L
diagnosis_by_age_UF_L <- lmer(UF_L_vol ~ anxiety_dose*PAT_AGE_AT_EXAM + (1|EMPI), data = df_demo_and_fascicles_no_unclass_anxiety_dose)
summary(diagnosis_by_age_UF_L)

### mean UF
diagnosis_by_age_mean_UF <- lmer(mean_UF_vol ~ anxiety_dose*PAT_AGE_AT_EXAM + (1|EMPI), data = df_demo_and_fascicles_no_unclass_anxiety_dose)
summary(diagnosis_by_age_mean_UF)
```
...
### Overall Disease by Anxiety Dose
```{r group_anxietydx_meds_and_dep_comorbidity}
#this collapses anxiety group + meds with those with anxiety group + meds + with/without depression
...
df_demo_and_fascicles_no_unclass_anxiety_dose <- df_demo_and_fascicles %>%
  ...
  #volume lost in anxiety net by anxiety dose*sex: 
  ...
  #volume lost in anxiety net by anxiety dose + sex + PAT_AGE_AT_EXAM: 
...
###Check for polynomial relationships - Not finding them w healthy, anxietydx OR meds, anxiety AND meds
...
########UF R - just looking at uncinate by anxiety dose; anxiety_dose      369.72     187.93   1.967   0.0499 *  *********** (LINEAR AGE)
  anxietyGrouping_UF_R_lm <- lm(UF_R_vol ~ anxiety_dose + PAT_AGE_AT_EXAM, data=df_demo_and_fascicles_no_unclass_anxiety_dose)
  summary(anxietyGrouping_UF_R_lm)
  
  #gam, anxiety_dose    357.0      188.8   1.891   0.0594 .
  ...
  #UF _ R anxiety_dose          anxiety_dose      377.35     188.43   2.003    0.046 *********   
  ...
  #gam, anxiety_dose    364.2      189.3   1.924   0.0552 .  
  ...
  #UF_L anxiety_dose:         anxiety_dose     232.210    133.294   1.742   0.0823 .  
  ...
  summary(anxietyGrouping_UF_L_by_sex_lm)
   ...
   #gam, anxiety_dose    229.9      133.9   1.717   0.0868 .  
...
## take home #2 : More disease in R & L uncinate in anxious versus nonanxious
  ...
  df_empi_dx_network_proportions <- df_demo_and_fascicles %>%
  filter(oanxietydx_OR_meds_AND_Anxietydx.Meds.And.Dep.True.Healthy.Collapsed != "unclassified") %>%
  filter(oanxietydx_OR_meds_AND_Anxietydx.Meds.And.Dep.True.Healthy.Collapsed == c("true_healthy", "Has.Anxiety.And.On.Anxiety.Meds.Inc.Dep")) %>% 
  mutate(subject = as.factor(EMPI)) %>%
  mutate(Diagnosis = oanxietydx_OR_meds_AND_Anxietydx.Meds.And.Dep.True.Healthy.Collapsed) %>%
  mutate(Diagnosis = ifelse(oanxietydx_OR_meds_AND_Anxietydx.Meds.And.Dep.True.Healthy.Collapsed=="true_healthy", "MS-Anxiety", "MS+Anxiety")) %>%
  mutate(Diagnosis = as.factor(Diagnosis)) %>%
...
##NetworkNonanxiety_network                      -0.254570   0.013575  92.000000 -18.753  < 2e-16 ***
##DiagnosisMS+Anxiety                             0.108710   0.040130 103.092356   2.709  0.00791 ** 
...
###do anova instead
##Network             1  3.059  3.0587  82.472  < 2e-16 ***
##Diagnosis           1  0.473  0.4728  12.749 0.000455 ***

disease_within_vs_outside_anxiety_net_by_dx_groups_anova <- aov(proportion_affected ~ Network+Diagnosis + Network:Diagnosis + PAT_AGE_AT_EXAM, data = df_empi_dx_network_proportions)
summary(disease_within_vs_outside_anxiety_net_by_dx_groups_anova)

#do lm UF*Diagnosis
 #UF_R_vol -> DiagnosisMS+Anxiety  1406.32     350.74   4.010 8.81e-05 ***
disease_UF_R_vol_lm <- lm(UF_R_vol ~ Diagnosis + PAT_AGE_AT_EXAM, data = df_empi_dx_network_proportions)
summary(disease_UF_R_vol_lm)

 #UF_L_vol -> DiagnosisMS+Anxiety   728.93     249.71   2.919  0.00395 ** 
disease_UF_L_vol_lm <- lm(UF_L_vol ~ Diagnosis + PAT_AGE_AT_EXAM, data = df_empi_dx_network_proportions)
summary(disease_UF_L_vol_lm)

## net by sex 

#simple lmer
#NetworkNonanxiety_network         -0.250361   0.011772  92.000001 -21.268  < 2e-16 ***
...
## diagnosis by sex 
 #simple lmer, DiagnosisMS+Anxiety         0.124327   0.040488   3.071  0.00246 **
...
#network * diagnosis + sex
 #simple lmer: NetworkNonanxiety_network                      -0.254570   0.013575  92.000000 -18.753  < 2e-16 ***
 ##DiagnosisMS+Anxiety                             0.107571   0.040361 101.820059   2.665  0.00895 ** 
