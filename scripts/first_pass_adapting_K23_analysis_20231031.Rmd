...
## Anxiety burden severity T test ranking
...
df_disease_burden_unc_and_anx_net_collapsed_variables <- df_demo_and_fascicles %>%
  filter(oAnxietydx.Meds.Inc.Dep.Collapsed != "unclassified") %>%
  dplyr::select(proportion_volume_lost_per_total_network_size_sum, proportion_volume_lost_anxiety_net_by_network_size_sum_top_25, proportion_volume_lost_nonanxiety_net_by_network_size_sum_bottom_75, UF_L_prop, UF_R_prop, oAnxietydx.Meds.Inc.Dep.Collapsed, osex) 
 

 tibble_overall_summaries <- df_disease_burden_unc_and_anx_net_collapsed_variables %>% 
  group_by(oAnxietydx.Meds.Inc.Dep.Collapsed) %>%
  summarize(n(),
            mean_disease_burden = mean(proportion_volume_lost_per_total_network_size_sum),
            mean_disease_burden_anxiety_net = mean(proportion_volume_lost_anxiety_net_by_network_size_sum_top_25),
            mean_disease_burden_nonanxiety_net = mean(proportion_volume_lost_nonanxiety_net_by_network_size_sum_bottom_75),
...
tibble_overall_summaries_by_sex <- df_disease_burden_unc_and_anx_net_collapsed_variables %>% 
  group_by(oAnxietydx.Meds.Inc.Dep.Collapsed, osex) %>%
  summarize(n(),
            mean_disease_burden = mean(proportion_volume_lost_per_total_network_size_sum),
            mean_disease_burden_anxiety_net = mean(proportion_volume_lost_anxiety_net_by_network_size_sum_top_25),
            mean_disease_burden_nonanxiety_net = mean(proportion_volume_lost_nonanxiety_net_by_network_size_sum_bottom_75),
  ...
  #mean disease
  anxietyGrouping_mean_disease_lm <- lm(proportion_volume_lost_per_total_network_size_sum ~ oAnxietydx.Meds.Inc.Dep.Collapsed, data=df_demo_and_fascicles)
  summary(anxietyGrouping_mean_disease_lm)
  
  anxietyGrouping_by_sex_mean_disease_lm <- lm(proportion_volume_lost_per_total_network_size_sum ~ oAnxietydx.Meds.Inc.Dep.Collapsed*osex, data=df_demo_and_fascicles)
  summary(anxietyGrouping_by_sex_mean_disease_lm)
  
  #anxiety network
  anxietyGrouping_anxiety_net_lm <- lm(proportion_volume_lost_anxiety_net_by_network_size_sum_top_25 ~ oAnxietydx.Meds.Inc.Dep.Collapsed, data=df_demo_and_fascicles)
  summary(anxietyGrouping_anxiety_net_lm)
  
  anxietyGrouping_by_sex_anxiety_net_lm <- lm(proportion_volume_lost_anxiety_net_by_network_size_sum_top_25 ~ oAnxietydx.Meds.Inc.Dep.Collapsed*osex, data=df_demo_and_fascicles)
  summary(anxietyGrouping_by_sex_anxiety_net_lm)
  
  #UF R
  anxietyGrouping_UF_R_lm <- lm(UF_R_vol ~ oAnxietydx.Meds.Inc.Dep.Collapsed, data=df_demo_and_fascicles)
  summary(anxietyGrouping_UF_R_lm)
  
  anxietyGrouping_UF_R_by_sex_lm <- lm(UF_R_vol ~ oAnxietydx.Meds.Inc.Dep.Collapsed*osex, data=df_demo_and_fascicles)
  summary(anxietyGrouping_UF_R_by_sex_lm)
  
  #UF L
  anxietyGrouping_UF_L_lm <- lm(UF_L_vol ~ oAnxietydx.Meds.Inc.Dep.Collapsed, data=df_demo_and_fascicles)
  summary(anxietyGrouping_UF_L_lm)
  
  anxietyGrouping_UF_L_by_sex_lm <- lm(UF_L_vol ~ oAnxietydx.Meds.Inc.Dep.Collapsed*osex, data=df_demo_and_fascicles)
  ...
  df_empi_dx_network_proportions <- df_demo_and_fascicles %>%
  #filter(oAnxietydx.Meds.Inc.Dep.Collapsed != "unclassified") %>%
  filter(oAnxietydx.Meds.Inc.Dep.Collapsed == c("healthy_ish", "Has.Anxiety.And.On.Anxiety.Meds.Inc.Dep")) %>% 
...
#  mutate(Diagnosis = oAnxietydx.Meds.Inc.Dep.Collapsed) %>%
  mutate(Diagnosis = ifelse(oAnxietydx.Meds.Inc.Dep.Collapsed=="healthy_ish", "MS-Anxiety", "MS+Anxiety")) %>%
  mutate(Diagnosis = as.factor(Diagnosis)) %>%
  #mutate(Has.anxietydx = as.factor(Has.anxietydx)) %>%
  #mutate(Anxiolytics_status = ifelse(On.Anxiolytics_no_beta_blocker==1, "On.Anxiolytics", "Off.Anxiolytics")) %>%
  #mutate(Anxiolytics_status = as.factor(Anxiolytics_status)) %>%
