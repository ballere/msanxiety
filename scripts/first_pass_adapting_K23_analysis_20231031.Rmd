...
########
fascicle_volumes_dep <- read.csv("/Users/eballer/BBL/msdepression/results/streamline_volume_within_dep_network.csv", header = T, sep = ",")
...
#lm
fascicle_number_of_anxiety_dx_lm <- lapply(fascicle_names_no_cranial_nerves, function(x) 
{
  lm(substitute(i ~ number_of_anxiety_comorbidities_extended, list(i = as.name(x))), data = df_demo_and_fascicles)
})
names(fascicle_number_of_anxiety_dx_lm) <- fascicle_names_no_cranial_nerves
...
#lm
fascicle_depANDanx_dx_lm <- lapply(fascicle_names_no_cranial_nerves, function(x) 
{
  lm(substitute(i ~ Has.depANDanx.dx, list(i = as.name(x))), data = df_demo_and_fascicles)
})
names(fascicle_depANDanx_dx_lm) <- fascicle_names_no_cranial_nerves
...
#lm
fascicle_number_of_psychiatric_dx_lm <- lapply(fascicle_names_no_cranial_nerves, function(x) 
{
  lm(substitute(i ~ number_of_psychiatric_comorbidities_extended, list(i = as.name(x))), data = df_demo_and_fascicles)
})
names(fascicle_number_of_psychiatric_dx_lm) <- fascicle_names_no_cranial_nerves
...
fascicle_volumes_yeo_7_binarized <- read.csv(paste0(homedir_msdep, "results/yeo_7_network_overlap_proportions_binarized.txt"), header = T, sep = ",")
...
new_df <- gsub(pattern = "_vol", replacement = "", x = names(df_demo_and_fascicles_prop_and_vol[grep(pattern = "_vol", x = names(df_demo_and_fascicles_prop_and_vol))]))
...
df_w_yeo_volume_info <- dep_and_healthy_groups_for_ICD_analysis_with_hydra_prop %>%
  filter(Hydra_k2 != -1)  %>% 
 # mutate(total_fascicle_vol_lost = rowMeans(across(all_of(fascicle_names_w_vol_suffix)))) %>%
  group_by(EMPI) %>% 
  arrange(EXAM_DATE) %>% 
  slice(1) %>% 
  dplyr::select(EMPI, Hydra_k2, ends_with("_vol")) %>%
  ungroup()

#add columns to data frame for each network
df_w_yeo_volume_info[,yeo_network_names] = 0

#this has been checked with eyeballs and works. You CAN trust the df_w_yeo_vol_info
for (network in yeo_network_names) {
  ...
  for (subj in 1:dim(df_w_yeo_volume_info)[1]) {
      ...
      "*df_w_yeo_volume_info[",subj,",2:(dim(df_w_yeo_volume_info)[2] - 7)])")
