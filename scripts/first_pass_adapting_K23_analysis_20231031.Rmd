...
bottom_75_total_volume_nondep <- sum(dep_net_overlap_from_bp_paper$non_zero_voxels_in_dep_map[(round(dim(dep_net_overlap_from_bp_paper)[1]/4,0) +1):dim(dep_net_overlap_from_bp_paper)[1]])
...
df_empi_dx_network_proportions <- df_demo_and_fascicles %>%
  ...
  mutate(Anxiolytics_status = ifelse(On.Anxiolytics_no_beta_blocker, "On.Anxiolytics", "Off.Anxiolytics")) %>%
  mutate(Anxiolytics_status = as.factor(Anxiolytics_status)) %>%
  mutate(Benzo_status = ifelse(On.Benzos, "On.Benzos", "Off.Benzos")) %>%
