...
df_demo_and_fascicles_no_unclass_anxiety_dose <- df_demo_and_fascicles %>%
  ...
  #volume lost in anxiety net by anxiety dose*sex: anxiety_dose         0.057403   0.020404   2.813  0.00517 ** 
  ...
  #gam, anxiety_dose, anxiety_dose      anxiety_dose         0.059732   0.020184   2.959  0.00328 **        
...
disease_UF_R_vol_lm <- lm(UF_R_vol ~ Diagnosis + osex + PAT_AGE_AT_EXAM, data = uncinate_by_dx_df)
summary(disease_UF_R_vol_lm)

 #UF_L_vol ->DiagnosisMS+Anxiety   729.21     389.80   1.871  0.06386 .    
disease_UF_L_vol_lm <- lm(UF_L_vol ~ Diagnosis + osex+PAT_AGE_AT_EXAM, data = uncinate_by_dx_df)
summary(disease_UF_L_vol_lm)

#bilat: DiagnosisMS+Anxiety   874.12     412.30   2.120   0.0361 * 
disease_mean_UF_vol_lm <- lm(mean_UF_vol ~ Diagnosis + osex+PAT_AGE_AT_EXAM, data = uncinate_by_dx_df)
summary(disease_mean_UF_vol_lm)


#gam, Fx= F
#UF_R_vol -> DiagnosisMS+Anxiety  1019.03     535.20   1.904   0.0593 .   
disease_UF_R_vol_gam <- gam(UF_R_vol ~ Diagnosis + osex + s(PAT_AGE_AT_EXAM, k = 4, fx = F), data = uncinate_by_dx_df)
summary(disease_UF_R_vol_gam)

 #UF_L_vol -> DiagnosisMS+Anxiety    675.7      381.3   1.772    0.079 .  
disease_UF_L_vol_gam <- gam(UF_L_vol ~ Diagnosis + osex+s(PAT_AGE_AT_EXAM, k = 4, fx = F), data = uncinate_by_dx_df)
...
# UF_L, P = NS
dep_UF_L_gam <- gam(UF_L_vol ~ depDiagnosis + osex + s(PAT_AGE_AT_EXAM, k=4, fx=F), data = df_dep)
summary(dep_UF_L_gam)

#UF_R, P = NS
dep_UF_R_gam <- gam(UF_R_vol ~ depDiagnosis + osex + s(PAT_AGE_AT_EXAM, k=4, fx=F), data = df_dep)
summary(dep_UF_R_gam)
...
df_demo_and_fascicles_no_unclass_anxiety_dose <- df_demo_and_fascicles %>%
  filter(oanxietydx_OR_meds_AND_Anxietydx.Meds.And.Dep.True.Healthy.Collapsed != "unclassified") %>%
  filter(anxiety_dose != -1) %>%
  mutate(mean_fornix_volume = (F_R_vol+F_L_vol)/2)


  #gam, Fornix R anxiety_dose  - NS 
...
### Diag Effect Size * Fascicle anxiety net overlap (from depression analysis) 
#### Make data frame 
...
#### Plot EfSize x Anxiety net overlap - N.S. 
``` {r plot_scatter}
#######################
### plot differences ##
#######################

#N.S., (including if I drop all fascicles with no overlap)
...
####. within/outside anxiety network #######
#dose, s(PAT_AGE_AT_EXAM):anxiety_dose 2.842  3.231 4.792 0.00242 **
...
#factor
#anxiety_dose_factor.L  0.09073    0.03830   2.369   0.0184 * 
#s(PAT_AGE_AT_EXAM):anxiety_dose_factor1 1.000  1.000 8.158 0.00453 **
#s(PAT_AGE_AT_EXAM):anxiety_dose_factor2 1.816  2.208 1.651 0.19973  
...
##dose
#s(PAT_AGE_AT_EXAM):anxiety_dose 3.19  3.577 5.659 0.000746 ***
...
#factor
#anxiety_dose_factor.L  0.07359    0.02950   2.495    0.013 * 
#s(PAT_AGE_AT_EXAM):anxiety_dose_factor1 1.703  2.071 4.247  0.0136 *
#s(PAT_AGE_AT_EXAM):anxiety_dose_factor2 2.003  2.403 2.034  0.1110  
...
#dose
#s(PAT_AGE_AT_EXAM):anxiety_dose 2.006  2.013 7.36 0.00071 ***
...
#factor, only sig is smooth
#s(PAT_AGE_AT_EXAM):anxiety_dose_factor1 1.127  1.242 12.824 0.000293 ***
#s(PAT_AGE_AT_EXAM):anxiety_dose_factor2 1.002  1.003  0.538 0.463889    
...
#uncinate left
#dose
#s(PAT_AGE_AT_EXAM):anxiety_dose   2      2 4.635  0.0103 *
...
#factor
#s(PAT_AGE_AT_EXAM):anxiety_dose_factor1 1.001  1.002 13.507 0.000271 ***
#s(PAT_AGE_AT_EXAM):anxiety_dose_factor2 1.007  1.014  0.611 0.432612    
...
#gam
fascicle_gam <- lapply(fascicle_names_top_25_vol_w_suff, function(x) 
{
  gam(substitute(i ~ oanxietydx_OR_meds_AND_Anxietydx.Meds.And.Dep.True.Healthy.Collapsed + osex+s(PAT_AGE_AT_EXAM, k=4, fx=F), list(i = as.name(x))), data = df_demo_and_fascicles_no_unclass_anxiety_dose)
})
names(fascicle_gam) <- fascicle_names_top_25_vol_w_suff
...
#gam
fascicle_gam <- lapply(fascicle_names_bottom_75_prop_w_suff, function(x) 
{
  gam(substitute(i ~ oanxietydx_OR_meds_AND_Anxietydx.Meds.And.Dep.True.Healthy.Collapsed + osex+s(PAT_AGE_AT_EXAM, k=4, fx=F), list(i = as.name(x))), data = df_demo_and_fascicles_no_unclass_anxiety_dose)
})
names(fascicle_gam) <- fascicle_names_bottom_75_prop_w_suff
