...
#  "MS+severeA" = "#440154" 
  ...
  "MS+mildA" = "#2d708e",
  "MS+severeA" = "#440154" 
 # "MS+mildA" = viridis::viridis(3, option = "C")[2],  # Use green from 'D' for MS+noA
#  "MS+severeA" = viridis::viridis(3, option = "D")[1],  # Use a different color from 'D'
#  "MS+noA" = viridis::viridis(3, option = "D")[2]  # Use the last color from 'D'
)

##### BOX PLOT NO JITTER
prop_injured_mean_uncinate_by_anxiety_burden_collapsed_vars_plot <- ggplot(df_disease_burden_unc_and_anx_net_collapsed_variables, aes(x = anxiety_severity, y = mean_UF_prop, color = anxiety_severity)) +
  geom_boxplot(aes(fill = anxiety_severity), alpha = 0.65, outlier.size = 0) +  # Add boxplots with some transparency
...
#  geom_jitter(width = 0.2, size = 2, alpha = 0.8) +  # Add jittered points for individual observations
  #scale_color_viridis(discrete = TRUE, option = "C", direction = -1) +  # Use Viridis color scale
...
prop_injured_mean_uncinate_by_anxiety_burden_collapsed_vars_plot_horiz <- ggplot(df_disease_burden_unc_and_anx_net_collapsed_variables, aes(y = anxiety_severity, x = mean_UF_prop, color = anxiety_severity)) +
  geom_boxplot(aes(fill = anxiety_severity), alpha = 0.65, outlier.size = 0) +  # Add boxplots with some transparency
#  geom_jitter(width = 0.2, size = 2, alpha = 0.8) +  # Add jittered points for individual observations
  scale_color_viridis(discrete = TRUE, option = "D", direction = -1) +  # Use Viridis color scale
  ...
  labs(x = "Percent (%) of Uncinate Injured")
prop_injured_mean_uncinate_by_anxiety_burden_collapsed_vars_plot_horiz

```


###### dot plot
```{r dot_plot_and_others}
