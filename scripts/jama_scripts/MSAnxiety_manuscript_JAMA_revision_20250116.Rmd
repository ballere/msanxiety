...
## anxiety diagnosis ##
#######################
df_anxiety_dx <- df_demo_and_fascicles %>%
  filter(oanxietydx_OR_meds_AND_Anxietydx.Meds.And.Dep.True.Healthy.Collapsed != "unclassified") %>%
  filter(oanxietydx_OR_meds_AND_Anxietydx.Meds.And.Dep.True.Healthy.Collapsed != "Has.Anxietydx.OR.Antianxiety.Meds") %>%
  mutate(subject = as.factor(EMPI)) %>%
  mutate(anxiety_Diagnosis = oanxietydx_OR_meds_AND_Anxietydx.Meds.And.Dep.True.Healthy.Collapsed) %>%
  mutate(anxiety_Diagnosis = ifelse(oanxietydx_OR_meds_AND_Anxietydx.Meds.And.Dep.True.Healthy.Collapsed=="true_healthy", "MS-Anxiety", "MS+Anxiety")) %>%
  mutate(anxiety_Diagnosis = as.factor(anxiety_Diagnosis)) %>%
  ungroup()
...
## anxiety dose ##
################## 
df_anxiety_dose <-  df_demo_and_fascicles %>%
  filter(oanxietydx_OR_meds_AND_Anxietydx.Meds.And.Dep.True.Healthy.Collapsed != "unclassified") %>%
  filter(anxiety_dose != -1) #n=372
...
# Model 1: depDiagnosis with df_dep
model_depDiagnosis <- gam(mean_UF_prop ~ depDiagnosis + s(PAT_AGE_AT_EXAM, k=4, fx=F) + osex, data = df_dep)

# Model 2: anxiety_Diagnosis with df_anxiety_dx
model_anxiety_Diagnosis <- gam(mean_UF_prop ~ anxiety_Diagnosis + s(PAT_AGE_AT_EXAM, k=4, fx=F) + osex, data = df_anxiety_dx)

# Model 3: anxiety_dose with df_anxiety_dose
model_anxiety_dose <- gam(mean_UF_prop ~ anxiety_dose + s(PAT_AGE_AT_EXAM, k=4, fx=F) + osex, data = df_anxiety_dose)

# Step 2: Extract residuals from each model (mean_UF_prop adjusted for age and sex)
residuals_depDiagnosis <- residuals(model_depDiagnosis)
residuals_anxiety_Diagnosis <- residuals(model_anxiety_Diagnosis)
residuals_anxiety_dose <- residuals(model_anxiety_dose)

# Step 3: Calculate the correlation between mean_UF_prop and the predictors (residuals)
cor_depDiagnosis <- cor(df_dep$mean_UF_prop, residuals_depDiagnosis)
cor_anxiety_Diagnosis <- cor(df_anxiety_dx$mean_UF_prop, residuals_anxiety_Diagnosis)
cor_anxiety_dose <- cor(df_anxiety_dose$mean_UF_prop, residuals_anxiety_dose)

# Print the correlation results
cat("Correlation between mean_UF_prop and depDiagnosis: ", cor_depDiagnosis, "\n")
cat("Correlation between mean_UF_prop and anxiety_Diagnosis: ", cor_anxiety_Diagnosis, "\n")
cat("Correlation between mean_UF_prop and anxiety_dose: ", cor_anxiety_dose, "\n")

# Step 4: Perform Steiger's test for comparing the correlations
# Create a function for Steiger's test (comparison of two correlated correlations)
steiger_test <- function(r12, r13, n) {
  # r12: correlation between variable1 and variable2
  # r13: correlation between variable1 and variable3
  # n: sample size
  
  # Calculate the Z score for comparison
  z <- (r12 - r13) / sqrt((1 - r12^2) / (n - 3) + (1 - r13^2) / (n - 3))
...
n = length(Reduce(intersect, list(df_dep$EMPI, df_anxiety_dx$EMPI, df_anxiety_dose$EMPI)))  

# Step 5: Test for differences between correlations
# Test if the correlation between anxiety_Diagnosis and mean_UF_prop is stronger than depression
steiger_test_anxiety_vs_dep <- steiger_test(cor_anxiety_Diagnosis, cor_depDiagnosis, n)

# Test if the correlation between anxiety_dose and mean_UF_prop is stronger than depression
steiger_test_anxiety_dose_vs_dep <- steiger_test(cor_anxiety_dose, cor_depDiagnosis, n)

# Output the results of Steiger's test; Z score:  -0.7806441 ; p-value:  0.4350118 
cat("Steiger's Test: Anxiety Diagnosis vs Depression Diagnosis\n")
cat("Z score: ", steiger_test_anxiety_vs_dep$Z_score, "\n")
cat("p-value: ", steiger_test_anxiety_vs_dep$p_value, "\n")

#Z score:  0.07445476 ; p-value:  0.9406485 
cat("Steiger's Test: Anxiety Dose vs Depression Diagnosis\n")
cat("Z score: ", steiger_test_anxiety_dose_vs_dep$Z_score, "\n")
cat("p-value: ", steiger_test_anxiety_dose_vs_dep$p_value, "\n")
```
