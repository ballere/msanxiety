...
data_final_rds <-data_pull %>% 
  ...
  mutate(On.DHODH_inhibitor = ifelse(str_detect(Medication, paste(msmeds$Medication[msmeds$Mechanism == "DHODH-inhibitor"], collapse = "|")), 1, 0)) %>%
  mutate(On.a4_integrin_inhibitor = ifelse(str_detect(Medication, paste(msmeds$Medication[msmeds$Mechanism == "a4_integrin-inhibitor"], collapse = "|")), 1, 0)) %>%
  mutate(On.anti_metabolite = ifelse(str_detect(Medication, paste(msmeds$Medication[msmeds$Mechanism == "anti-metabolite"], collapse = "|")), 1, 0)) %>%
  mutate(On.mbp_analog= ifelse(str_detect(Medication, paste(msmeds$Medication[msmeds$Mechanism == "mbp-analog"], collapse = "|")), 1, 0)) %>%
  mutate(On.nrf_2= ifelse(str_detect(Medication, paste(msmeds$Medication[msmeds$Mechanism == "nrf-2"], collapse = "|")), 1, 0)) %>%
  mutate(On.s1p_agonist= ifelse(str_detect(Medication, paste(msmeds$Medication[msmeds$Mechanism == "s1p-agonist"], collapse = "|")), 1, 0)) %>%
