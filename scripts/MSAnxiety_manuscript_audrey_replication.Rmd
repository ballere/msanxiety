...
df_demo_and_fascicles_no_unclass_anxiety_dose <- df_demo_and_fascicles %>%
  filter(oanxietydx_OR_meds_AND_Anxietydx.Meds.And.Dep.True.Healthy.Collapsed != "unclassified") %>%
  filter(anxiety_dose != -1) %>% #n=371
  mutate(has.uncinate.lesion = ifelse(mean_UF_vol >0, 1, 0)) #anywhere where there is *some* overlap counts as a 1 - they have an uncinate lesion, otherwise 0
...
#bilateral UF ~ Diagnosis: DiagnosisMS+Anxiety   816.97     404.08   2.022   0.0454 * 
...
#print summary stats
 gam_summary_stats(disease_mean_UF_vol_gam)
```
...
df_demo_and_fascicles_no_unclass_anxiety_dose <- df_demo_and_fascicles %>%
  ...
  #bilat anxiety dose + sex + age: anxiety_dose    383.3      184.3   2.080   0.0383 *  
  ...
  #print summary stats
 gam_summary_stats(anxietyGrouping_mean_UF_by_sex_gam)
...
```{r laterality}
  #laterality 
  #overall - t = -8.2035, df = 661.94, p-value = 1.217e-15
  ...
  #healthy - t = -4.3999, df = 171.31, p-value = 1.898e-05
  ...
  #anxious+meds,t = -2.261, df = 42.723, p-value = 0.02891
...
#mean UF, depDiagnosisMS+noDepression   -386.4      232.8  -1.660    0.098 .
dep_mean_UF_gam <- gam(mean_UF_vol ~ depDiagnosis + osex + s(PAT_AGE_AT_EXAM, k=4, fx=F), data = df_dep)
summary(dep_mean_UF_gam)

  #print summary stats
 gam_summary_stats(dep_mean_UF_gam)
```

###### Fornix by Anxiety Diagnosis and Dose (N.S)
...
##### healthy vs sick
#Fornix gam Diagnosis; DiagnosisMS+Anxiety    283.0      375.6   0.753   0.4528   - NS
anxietyDiagnosis_mean_F_by_sex_gam <- gam(mean_fornix_volume ~ Diagnosis + osex + s(PAT_AGE_AT_EXAM, k = 4, fx = T), data = df_empi_dx_network_proportions)
summary(anxietyDiagnosis_mean_F_by_sex_gam)

 #print summary stats
 gam_summary_stats(anxietyDiagnosis_mean_F_by_sex_gam)
...
df_demo_and_fascicles_no_unclass_anxiety_dose <- df_demo_and_fascicles %>%
  ...
  #gam, Fornix anxiety_dose    173.1      166.7   1.038    0.300   - NS
 ...
 #print summary stats
 gam_summary_stats(anxietyGrouping_mean_F_by_sex_gam)
```
...
#bilateral UF ~ Diagnosis: DiagnosisMS+Anxiety    718.8      413.2    1.74   0.0846 .   trend, likely due to sample size
disease_mean_UF_vol_gam <- gam(mean_UF_vol ~ Diagnosis + osex+s(PAT_AGE_AT_EXAM, k = 4, fx = F), data = uncinate_by_dx_df)
summary(disease_mean_UF_vol_gam)
...
df_demo_and_fascicles_no_unclass_anxiety_dose <- df_demo_and_fascicles_no_steroids_or_ifn %>%
  ...
  #bilat anxiety dose + sex + age: anxiety_dose: anxiety_dose    358.3      188.3   1.903   0.0578 .   (trend, likely due to sample size)   
  anxietyGrouping_mean_UF_by_sex_gam <- gam(mean_UF_vol ~ anxiety_dose + osex + s(PAT_AGE_AT_EXAM, k = 4, fx = F), data=df_demo_and_fascicles_no_unclass_anxiety_dose)
  summary(anxietyGrouping_mean_UF_by_sex_gam)
...
# mean uncinate L - osex.L          -152.317    174.353  -0.874    0.383 ns
...
#overall - DiagnosisMS+Anxiety  0.03966    0.02329   1.703   0.0893 .  NS for diagnosis
...
#s(PAT_AGE_AT_EXAM):DiagnosisMS-Anxiety 1.581  1.930 5.026 0.01362 * 
#s(PAT_AGE_AT_EXAM):DiagnosisMS+Anxiety 1.994  2.385 5.249 0.00313 **
...
summary(diagnosis_by_age_gam)$s.table
  ...
  #print summary stats
 gam_summary_stats(diagnosis_by_age_gam)
...
#uncinate mean, DiagnosisMS+Anxiety    420.3      224.9   1.869   0.0624 . NS
#s(PAT_AGE_AT_EXAM):DiagnosisMS-Anxiety 1.007  1.014 23.98 2.07e-06 ***
#s(PAT_AGE_AT_EXAM):DiagnosisMS+Anxiety 1.000  1.000 17.15 4.36e-05 ***
...
#plot(diagnosis_by_age_UF_L_gam)
  ...
  #print summary stats
 gam_summary_stats(diagnosis_by_age_mean_UF_gam)
...
#overall - osex.L                  0.066402   0.075006   0.885  0.37658  NS
...
#gam dose; s(PAT_AGE_AT_EXAM):anxiety_dose 2.058  2.452 4.972 0.00315 **
...
#anxiety_dose_factor.L  0.07788    0.03209   2.427   0.0157 * 
#s(PAT_AGE_AT_EXAM):anxiety_dose_factor1 1.459  1.765 5.736  0.0138 *
#s(PAT_AGE_AT_EXAM):anxiety_dose_factor2 1.972  2.372 2.128  0.1123  
...
#plot(diagnosis_by_age_gam_factor)
  ...
  #print summary stats
 gam_summary_stats(diagnosis_by_age_gam_factor)
...
#uncinate mean
#dose
#s(PAT_AGE_AT_EXAM):anxiety_dose 1.003  1.005 12.59 0.000427 ***
...
#factor
#s(PAT_AGE_AT_EXAM):anxiety_dose_factor1 1.002  1.004 17.462 3.69e-05 ***
#s(PAT_AGE_AT_EXAM):anxiety_dose_factor2 1.002  1.005  0.029    0.872 
...
#plot(diagnosis_by_age_UF_L_gam_factor)
  ...
  #print summary stats
 gam_summary_stats(diagnosis_by_age_mean_UF_gam_factor)
...
#fdr corrected (18 items correct)
...
#bilateral UF ~ Diagnosis: DiagnosisMS+Anxiety   -187.4      885.1  -0.212    0.835 N.S.
disease_mean_UF_vol_gam <- gam(mean_UF_vol ~ Diagnosis + osex+s(PAT_AGE_AT_EXAM, k = 4, fx = F), data = uncinate_by_dx_df)
summary(disease_mean_UF_vol_gam)
...
df_demo_and_fascicles_no_unclass_anxiety_dose <- df_people_with_promis_data_scores_added_unique %>%
  ...
  #bilat anxiety dose -anxiety_dose   -46.71     423.41  -0.110 0.912524   NS
  anxietyGrouping_mean_UF_by_sex_gam <- gam(mean_UF_vol ~ anxiety_dose + osex + s(PAT_AGE_AT_EXAM, k = 4, fx = F), data=df_demo_and_fascicles_no_unclass_anxiety_dose)
  summary(anxietyGrouping_mean_UF_by_sex_gam)
